...
def custom_fullplot():
    ...
    line = np.arange(0,301,10)
    plt.plot(line[8:],np.ones(len(line[8:]))*3e-11,label='CCP metric >80 amu <3E-11 Torr',c='c')
    plt.plot(line[15:],np.ones(len(line[15:]))*3e-12,label='CCP metric >150 amu <3E-12 Torr',c='m')
    plt.axvline(x=80,c='c')
    plt.axvline(x=150,c='m')
...
if comp_q=='y':
    ...
    plt.show()
    plt.figure()
    plt.plot(amu[second],(pp[second]-pp[first]),label='{num}'.format(num=second_t),c='k')
    plt.axvline(x=80,c='c')
    plt.axvline(x=150,c='m')
    annotation = "Time Between Scans {t} Hours".format(t=tbtw)
    plt.plot([], [], ' ', label=annotation)
    plt.title('Difference plot of '+str(first_t)+' and '+str(second_t))
    plt.xlabel('AMU')
...
def comp_3():
    ...
    first_t=input('Input first timestamp (copy paste from above)\n')
    second_t=input('Input second timestamp (copy paste from above)\n')
    third_t=input('Input third timestamp (copy paste from above)\n')
    first = np.where(ht_arr==first_t)[0][0]
    second = np.where(ht_arr==second_t)[0][0]
